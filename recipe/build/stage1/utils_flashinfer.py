...
def supports_trtllm_attention(is_prefill: bool = False) -> bool:
    ...
    if current_platform.is_device_capability_family(100):  # NVFP4KV-PORT
        return True  # NVFP4KV-PORT
    # SM120/SM121 (consumer Blackwell, GB10): XQA decode only; prefill uses  # NVFP4KV-PORT
    # fa2 (trtllm-gen FMHA is SM100-only). Enables the FA2 NVFP4-KV decode  # NVFP4KV-PORT
    # route via supports_kv_cache_dtype/backend gating.  # NVFP4KV-PORT
    if current_platform.is_device_capability_family(120):  # NVFP4KV-PORT
        return not is_prefill  # NVFP4KV-PORT
    return False  # NVFP4KV-PORT
