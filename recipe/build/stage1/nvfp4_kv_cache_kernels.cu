...
void reshape_and_cache_nvfp4_dispatch(
  ...
  // paged reader, which reads V scales linearly; the SM100 trtllm-gen reader
  // keeps its 4-token V scale swizzle. Both consume the same per-page
  // [K_data | K_scale | V_data | V_scale] layout, so only the V-scale swizzle
  // is arch-conditional.
