...
class FlashInferBackend(AttentionBackend):
    ...
    def supports_kv_cache_dtype(cls, kv_cache_dtype: CacheDType | None) -> bool:
        if kv_cache_dtype == "nvfp4":
            # Consumer Blackwell (sm120/sm121, GB10): NVFP4 KV is served  # NVFP4KV-PORT
            # through the FlashInfer FA2 paged reader (uint8 fp4 cache), no  # NVFP4KV-PORT
            # trtllm-gen requirement. Mirrors the builder __init__ gate.  # NVFP4KV-PORT
            if current_platform.is_device_capability_family(120):  # NVFP4KV-PORT
                return True  # NVFP4KV-PORT
            return (
    ...
    def get_required_kv_cache_layout(cls) -> KVCacheLayoutType | None:
        ...
        if capability is not None and capability.major == 10:
            return "HND"
        # NVFP4 KV on consumer Blackwell (sm120/sm121, FA2 path): each K/V  # NVFP4KV-PORT
        # side of the cache packs [data | scale] regions carved out of the  # NVFP4KV-PORT
        # side's byte range (reshape_and_cache_nvfp4 writes the scales;  # NVFP4KV-PORT
        # nvfp4_split_data_scale reads them back with derived strides). That  # NVFP4KV-PORT
        # carve is only byte-coherent under head-major HND. This hook has no  # NVFP4KV-PORT
        # dtype parameter, so read the ambient vllm config (same pattern as  # NVFP4KV-PORT
        # get_kv_connector_cache_layout).  # NVFP4KV-PORT
        if capability is not None and capability.major == 12:  # NVFP4KV-PORT
            vllm_config = get_current_vllm_config_or_none()  # NVFP4KV-PORT
            if (  # NVFP4KV-PORT
                vllm_config is not None  # NVFP4KV-PORT
                and vllm_config.cache_config is not None  # NVFP4KV-PORT
                and vllm_config.cache_config.cache_dtype == "nvfp4"  # NVFP4KV-PORT
            ):  # NVFP4KV-PORT
                return "HND"  # NVFP4KV-PORT
        return None
...
def _vllm_nvfp4_a4q_enabled() -> bool:  # A4Q-PORT
    # A4Q (native NVFP4 QKᵀ MMA): opt-in via VLLM_NVFP4_A4Q. Read from the raw
    # environment (this module does not register a typed vLLM env for it) so the
    # flag is a pure no-op when unset. Only meaningful on the FA2 NVFP4-KV lane.
    import os  # A4Q-PORT

    return os.environ.get("VLLM_NVFP4_A4Q", "0") not in ("", "0")  # A4Q-PORT
...
class FlashInferMetadataBuilder(AttentionMetadataBuilder[FlashInferMetadata]):
    ...
    def __init__(
        ...
        if self.kv_cache_spec.kv_quant_mode != KVQuantMode.NONE:
            ...
            self.use_fa2_nvfp4_kv = False  # NVFP4KV-PORT
            if self.is_kvcache_nvfp4:
                if current_platform.is_device_capability_family(120):  # NVFP4KV-PORT
                    # Consumer Blackwell (sm120/sm121, GB10): no trtllm-gen  # NVFP4KV-PORT
                    # FP4 FMHA, so route NVFP4 KV through FlashInfer's FA2  # NVFP4KV-PORT
                    # paged reader. The cache stores packed uint8 fp4 data;  # NVFP4KV-PORT
                    # per-side [data | scale] regions are read as strided  # NVFP4KV-PORT
                    # views (nvfp4_split_data_scale).  # NVFP4KV-PORT
                    self.use_fa2_nvfp4_kv = True  # NVFP4KV-PORT
                elif (
                    ...
                    raise ValueError(
                        ...
                        "FlashInfer path or consumer Blackwell (sm120/sm121)."  # NVFP4KV-PORT
                    )
                # For NVFP4, kv_cache_dtype stays as the string "nvfp4"
                # which is passed to FlashInferImpl. The FA2 path substitutes
                # torch.uint8 at each plan() site (see build()).
        ...
        if self.is_kvcache_nvfp4 and get_kv_cache_layout() != "HND":  # NVFP4KV-PORT
            # The NVFP4 per-side [data | scale] carve is only byte-coherent  # NVFP4KV-PORT
            # under head-major HND (see get_required_kv_cache_layout); NHD  # NVFP4KV-PORT
            # would silently corrupt the cache, so fail at init instead.  # NVFP4KV-PORT
            raise ValueError(  # NVFP4KV-PORT
                "NVFP4 KV cache requires the HND KV cache layout; resolved "  # NVFP4KV-PORT
                f"layout is {get_kv_cache_layout()!r}. Unset "  # NVFP4KV-PORT
                "VLLM_KV_CACHE_LAYOUT or set it to 'HND'."  # NVFP4KV-PORT
            )  # NVFP4KV-PORT

        # A4Q-PORT: nvf4 block-scaled QKᵀ MMA on the FA2 NVFP4-KV lane
        # (env-gated VLLM_NVFP4_A4Q). Strict subset of use_fa2_nvfp4_kv: when
        # the FA2 lane is off the flags self-disable and the stock path runs
        # unchanged. The query is quantized to NVFP4 inside the FlashInfer FA2
        # wrapper.run() (nvfp4_quantize_q_cuda) once plan() records use_nvf4_qk,
        # so nothing else in the run path changes. TARGET has no VO-split, so
        # only head_dim and DCP gate here (SOURCE additionally gated vo_split
        # geometry; the FA2 lane raises on DCP anyway).
        self.a4q_prefill = (  # A4Q-PORT
            _vllm_nvfp4_a4q_enabled()  # A4Q-PORT
            and self.use_fa2_nvfp4_kv  # A4Q-PORT
            and self.head_dim in (128, 256, 512)  # A4Q-PORT
            and not self.use_dcp  # A4Q-PORT
        )  # A4Q-PORT
        # A4Q-PORT: decode (fa2 tensor-core route) supports head_dim {128, 256}.
        self.a4q_decode = self.a4q_prefill and self.head_dim in (128, 256)  # A4Q-PORT
        if self.a4q_prefill:  # A4Q-PORT
            logger.info_once(  # A4Q-PORT
                "A4Q: nvf4 block-scaled QK MMA enabled for FA2 NVFP4 prefill "  # A4Q-PORT
                "(decode=%s, head_dim=%d).",  # A4Q-PORT
                self.a4q_decode,  # A4Q-PORT
                self.head_dim,  # A4Q-PORT
            )  # A4Q-PORT
        elif _vllm_nvfp4_a4q_enabled() and self.use_fa2_nvfp4_kv:  # A4Q-PORT
            logger.info_once(  # A4Q-PORT
                "A4Q requested but out of scope (head_dim=%d, dcp=%s); using "  # A4Q-PORT
                "the standard FA2 NVFP4 prefill path.",  # A4Q-PORT
                self.head_dim,  # A4Q-PORT
                self.use_dcp,  # A4Q-PORT
            )  # A4Q-PORT
        elif _vllm_nvfp4_a4q_enabled():  # A4Q-PORT
            logger.info_once(  # A4Q-PORT
                "A4Q requested (VLLM_NVFP4_A4Q) but no FA2 NVFP4-KV lane on "  # A4Q-PORT
                "this device/dtype; A4Q is inert."  # A4Q-PORT
            )  # A4Q-PORT
        ...
        # SM120/SM121 have no trtllm-gen/XQA decode kernel, so NVFP4 KV decode  # NVFP4KV-PORT
        # must go through the FA2 native decode wrapper.  # NVFP4KV-PORT
        self.use_trtllm_decode_attention = can_use_xqa_or_trtllm_gen_decode and (  # NVFP4KV-PORT
            not current_platform.is_device_capability_family(120)  # NVFP4KV-PORT
        )  # NVFP4KV-PORT
        self.flashinfer_trtllm_api_decode_kernel: FlashInferDecodeKernel | None = (
            self._get_flashinfer_trtllm_api_decode_kernel()
            if self.use_trtllm_decode_attention  # NVFP4KV-PORT
            else None
        )
        if (
            self.use_dcp
    ...
    def get_q_data_type(self, is_prefill: bool) -> torch.dtype:
        ...
        if cache_dtype == "nvfp4":
            if current_platform.is_device_capability_family(120):  # NVFP4KV-PORT
                # The FA2 paged nvfp4 reader (consumer Blackwell) consumes  # NVFP4KV-PORT
                # model-dtype queries; FP8-Q is a trtllm-gen-only contract.  # NVFP4KV-PORT
                return self.model_config.dtype  # NVFP4KV-PORT
    ...
    @staticmethod
    def _get_flashinfer_trtllm_api_decode_kernel() -> FlashInferDecodeKernel | None:  # NVFP4KV-PORT
        if current_platform.is_device_capability(90):
            return FlashInferDecodeKernel.XQA
        if current_platform.is_device_capability_family(120):  # NVFP4KV-PORT
            # Consumer Blackwell has no trtllm-gen/XQA decode; FA2 native.  # NVFP4KV-PORT
            return None  # NVFP4KV-PORT
    ...
    def _get_prefill_wrapper(
        ...
        if self._prefill_wrapper is None:
            if self.use_dcp:
                ...
                # NVFP4 KV: FlashInfer FA2 paged reader on consumer Blackwell  # NVFP4KV-PORT
                # (sm120/sm121, GB10); trtllm-gen on sm100f.  # NVFP4KV-PORT
                if self.use_fa2_nvfp4_kv:  # NVFP4KV-PORT
                    backend = "fa2"  # NVFP4KV-PORT
                elif self.is_kvcache_nvfp4:  # NVFP4KV-PORT
                    backend = "trtllm-gen"  # NVFP4KV-PORT
                else:  # NVFP4KV-PORT
                    backend = "auto"  # NVFP4KV-PORT
    ...
    def build(
        ...
        if use_cascade:
            ...
            attn_metadata.cascade_wrapper.plan(
                ...
                kv_data_type=(  # NVFP4KV-PORT
                    torch.uint8 if self.use_fa2_nvfp4_kv else self.kv_cache_dtype  # NVFP4KV-PORT
                ),  # NVFP4KV-PORT
            )
            ...
            if prefill_use_trtllm:
                ...
                if self.use_dcp:
                    ...
                    prefill_wrapper.plan(
                        ...
                        logits_soft_cap=self.logits_soft_cap,
                        q_data_type=self.q_data_type_prefill,
                        kv_cache_dtype=(  # NVFP4KV-PORT
                            torch.uint8  # NVFP4KV-PORT
                            if self.use_fa2_nvfp4_kv  # NVFP4KV-PORT
                            else self.kv_cache_dtype  # NVFP4KV-PORT
                        ),  # NVFP4KV-PORT
                    ...
                    # SM120 FA2 outputs model dtype directly; SM100 trtllm-gen  # NVFP4KV-PORT
                    # only supports FP8 output and needs dequantization.  # NVFP4KV-PORT
                    o_dtype = (
                        self.model_config.dtype  # NVFP4KV-PORT
                        if (self.is_kvcache_nvfp4 and self.use_fa2_nvfp4_kv)  # NVFP4KV-PORT
                        else FP8_DTYPE  # NVFP4KV-PORT
                        if self.is_kvcache_nvfp4  # NVFP4KV-PORT
                        else self.model_config.dtype
                    )
                    # A4Q-PORT: inject use_nvf4_qk only when A4Q is active so
                    # the stock FlashInfer plan() signature (no such kwarg) is
                    # untouched on every non-A4Q prefill (fp16/fp8/nvfp4-noqk).
                    a4q_prefill_kwargs = (  # A4Q-PORT
                        {"use_nvf4_qk": True} if self.a4q_prefill else {}  # A4Q-PORT
                    )  # A4Q-PORT
                    prefill_wrapper.plan(
        ...
        if num_decodes > 0:
            if decode_with_flashinfer_trtllm_api:
                ...
                fast_plan_decode(
                    ...
                    use_nvf4_qk=self.a4q_decode,  # A4Q-PORT
                )
...
class FlashInferImpl(AttentionImpl):
    ...
    def __init__(
        ...
        self.use_fa2_nvfp4_kv = (  # NVFP4KV-PORT
            self.is_kvcache_nvfp4  # NVFP4KV-PORT
            and current_platform.is_device_capability_family(120)  # NVFP4KV-PORT
        )  # NVFP4KV-PORT
        ...
        # The sm12x FA2 path writes into `output` directly and never reads this.  # NVFP4KV-PORT
        if (
            self.is_kvcache_nvfp4
            and vllm_config is not None
            and not self.use_fa2_nvfp4_kv  # NVFP4KV-PORT
        ):
        ...
        if num_prefill_tokens > 0:
            ...
            if not prefill_use_trtllm:
                ...
                if use_dcp:
                    ...
                    # afterwards.
                    # SM120 FA2 outputs model dtype directly, no intermediate.  # NVFP4KV-PORT
...
def fast_plan_decode(
    ...
    q_len_per_req: int = 1,
    use_nvf4_qk: bool = False,  # A4Q-PORT
) -> None:
    """
    ...
    if not self.is_cuda_graph_enabled or getattr(self, "vllm_first_call", True):
        # A4Q-PORT: thread use_nvf4_qk into the wrapper's real plan() (the fork
        # BatchDecodeWithPagedKVCacheWrapper.plan accepts it and records
        # self._use_nvf4_qk, which run() reads to quantize the query). Injected
        # only when active so the stock plan() signature is untouched otherwise.
        # This warm-up plan builds the a4q _cached_module reused by the
        # cudagraph fast path below, so fast_decode_plan needs no kwarg.
        a4q_decode_kwargs = {"use_nvf4_qk": True} if use_nvf4_qk else {}  # A4Q-PORT
        self.plan(
            indptr=indptr_cpu,
            indices=indices,
