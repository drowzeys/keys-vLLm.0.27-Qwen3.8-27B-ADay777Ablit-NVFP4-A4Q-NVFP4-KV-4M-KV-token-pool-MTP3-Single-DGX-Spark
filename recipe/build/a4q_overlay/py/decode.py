...
class BatchDecodeWithPagedKVCacheWrapper:
        ...
        *,
        ...
        if self._backend == "cute-dsl":
            ...
            if self._jit_module is not None:
                ...
                if use_nvf4_qk:
                    # A4Q decode is forced onto the tensor-core (=batch-prefill) route,
                    # so it reuses the exact fp4-QK kernel. Validate the same constraints.
                    if not self.use_tensor_cores:
                        raise ValueError(
                            "use_nvf4_qk requires use_tensor_cores=True on the decode wrapper"
                        )
                    if (
                        PosEncodingMode[pos_encoding_mode].value
                    ...
                    if head_dim not in (128, 256):
                        raise ValueError("use_nvf4_qk requires head_dim in {128, 256}")
    ...
    def run(
        ...
        *args
        ...
        # A4Q: quantize Q to packed e2m1 + per-16 ue4m3 SF on the fly (decode reuses the
        # tensor-core batch-prefill fp4-QK kernel). `out` is already allocated above.
        maybe_q_sf = None
        if getattr(self, "_use_nvf4_qk", False):
            ...
            if q.dtype in (torch.bfloat16, torch.float16):
                ...
                # Present the packed uint8 Q ([..., HEAD_DIM/2] bytes/row) as a DTypeQ
                # view so the fp4-QK kernel's DTypeQ-pointer element strides land on the
                # right rows. Without this the packed uint8 (1-byte) strides applied to a
                # 2-byte pointer address every (token, head) Q row at 2x its byte offset
                # (prefill Bug 2 analog; decode had the same gap). The 2x OOB read is
                # benign in eager (reads into adjacent pool memory -> cos ~0.93) but
                # faults as an illegal memory access under cudagraph capture.
        ...
        if self.use_tensor_cores:
            ...
            if getattr(self, "_use_nvf4_qk", False):
                # paged_run's trailing params are (..., uses_shared_paged_kv_idx,
                # multi_ctas_kv_counter_buffer, maybe_q_sf); the decode fa2 run_args stop
                # at uses_shared_paged_kv_idx, so pad the multi_ctas slot before maybe_q_sf.
                run_args += [None, maybe_q_sf]
