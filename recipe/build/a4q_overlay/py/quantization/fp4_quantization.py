...
def get_fp4_kv_quantization_module():
    ...
    # A4Q: Q quantization for the native nvf4 QK MMA path (no global scale;
    # per-16 e4m3 SF = amax/6, RNE e2m1 codes). Same JIT module as nvfp4_kv_quant.
    @register_custom_op(
...
def nvfp4_quantize_q_cuda(q: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    Per 16 consecutive elements along the last dim: ``sf = e4m3(amax / 6)``
    (amax clamped to >= 1e-6), codes are round-to-nearest-even e2m1 of
    ``x / float(sf)`` (PTX ``cvt.rn.satfinite.e2m1x2.f32``), packed 2 codes per
    byte, low nibble = even element. Requires SM100+.

    Returns ``(packed, sf)``: ``packed`` uint8 ``[..., head_dim // 2]`` and
    ``sf`` uint8 (e4m3 bytes) ``[..., head_dim // 16]``.
    """
    shape = q.shape
    D = shape[-1]
    if D % _NVFP4_BLOCK_SIZE != 0:
        raise ValueError(f"head_dim ({D}) must be divisible by {_NVFP4_BLOCK_SIZE}")
