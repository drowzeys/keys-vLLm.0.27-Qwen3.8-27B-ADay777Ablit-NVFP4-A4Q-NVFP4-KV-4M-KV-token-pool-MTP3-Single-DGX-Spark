...
from ..core import (
    ...
    logger,
    sm90a_nvcc_flags,
    sm120f_nvcc_flags,  # A4Q: compute_120f gencode (runs on sm_120a and sm_121a/GB10)
...
def gen_batch_prefill_module(
    ...
    if backend == "fa2":
        ...
        variant_decl = "#include<flashinfer/attention/variants.cuh>"
        if use_nvf4_qk:
            # A4Q: packed ue4m3 Q scale factors, appended LAST so the trailing
            # maybe_q_sf run-arg / params-struct slot lines up (see decode/prefill run()).
...
def gen_customize_batch_prefill_module(
    ...
    if use_nvf4_qk:
        # A4Q requires fp4 KV (the QK MMA static_asserts fp4 KV) and the Q SF tensor.
        if not require_fp4_kv_cache:
            raise ValueError(
                "use_nvf4_qk (A4Q native fp4 QK MMA) requires an NVFP4 KV cache "
                "(dtype_kv == __nv_fp4x2_e2m1)."
            )
        if "maybe_q_sf" not in additional_tensor_names:
            raise ValueError(
                "use_nvf4_qk requires the maybe_q_sf additional tensor (packed ue4m3 "
                "Q scale factors); pass it as the trailing additional tensor."
            )
    ...
    if backend == "auto":
        ...
        if use_nvf4_qk:
            # A4Q: the native nvf4 block-scale QK MMA (mma.sync…kind::mxf4nvf4…) needs
            # the compute_120f family target (runs on sm_120a AND sm_121a/GB10; a
            # sm_120a-only target fails "no kernel image" on the Spark) plus the
            # -DFLASHINFER_ENABLE_NVF4_QK_MMA gate that lights up the inline PTX.
            # sm120f_nvcc_flags already carries FLASHINFER_ENABLE_FP4_E2M1.
