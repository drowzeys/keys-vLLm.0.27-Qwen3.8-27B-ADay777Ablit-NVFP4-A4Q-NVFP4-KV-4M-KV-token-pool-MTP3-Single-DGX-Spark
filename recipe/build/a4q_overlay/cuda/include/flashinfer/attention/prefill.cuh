...
// A4Q: per-token/head packed ue4m3 scale factors for the fp4-quantized Q operand.
...
struct KernelTraits {
  ...
  // A4Q: when true, Q.K^T is computed with the native sm120 nvf4 block-scaled MMA
  // (compute_qk_nvf4). Requires fp4 KV; K is staged dense-packed (kDenseKFp4).
  static constexpr bool USE_NVF4_QK = USE_NVF4_QK_;
  // Dense-pack the fp4 K tile (row-major HEAD_DIM_QK/2 bytes/row, no swizzle) into the
  // FRONT of k_smem so compute_qk_nvf4's dense addressing matches the producer. Only the
  // K-write / K-read pattern changes; the k_smem allocation itself is unchanged (its full
  // half-empty size CTA_TILE_KV*HEAD_DIM_QK bytes >= the dense CTA_TILE_KV*HEAD_DIM_QK/2).
  static constexpr bool kDenseKFp4 = USE_NVF4_QK_ && is_fp4_type_v<DTypeKV_>;
...
__device__ __forceinline__ void produce_kv(smem_t<KTraits::SWIZZLE_MODE_KV> smem,
  ...
  // A4Q (J-5): dense-pack the fp4 K tile (single/ragged path). Store each K row as a
  // contiguous HEAD_DIM_QK/2-byte run (row-major, no swizzle) in the FRONT of k_smem so
  // compute_qk_nvf4's dense addressing matches. Reuses the exact k128B thread mapping and
  // running-GMEM pointer + 64b cp.async (cp-size 8 writes exactly 8 bytes, no neighbor
  // clobber); only the SMEM destination changes. Read side is compute_qk_nvf4.
  if constexpr (KTraits::kDenseKFp4 && !produce_v) {
    ...
    const uint32_t L = lane_idx % 8;                    // 8-byte feat chunk within a group
...
__device__ __forceinline__ void page_produce_kv(SmemStorage* smem_storage, uint32_t* smem_offset,
  ...
  // A4Q (J-5): dense-pack fp4 K into the front of k_smem (see produce_kv). GMEM side reuses
  // the exact thr_local_kv_offset mapping; only the SMEM destination byte offset changes.
  if constexpr (KTraits::kDenseKFp4 && !produce_v) {
    constexpr uint32_t ROW_BYTES = KTraits::HEAD_DIM_QK / 2;
    constexpr uint32_t NUM_D_ITERS = NUM_MMA_D / (8 / sizeof(DType));
...
__device__ __forceinline__ void page_produce_kv_on_the_fly(
  ...
  // A4Q (J-5): dense-pack fp4 K into the front of k_smem (see produce_kv). GMEM side reuses
  // the exact per-logical-row offset; only the SMEM destination byte offset changes.
  if constexpr (KTraits::kDenseKFp4 && !produce_v) {
...
#pragma unroll
    for (uint32_t i = 0; i < NUM_MMA_KV * ROWS_PER_ITER / NUM_WARPS_Q; ++i) {
      ...
      uint8_t* drow = dense_base + logical_row * ROW_BYTES;
#pragma unroll
      for (uint32_t j = 0; j < NUM_D_ITERS; ++j) {
        cp_async::pred_load_128b_from_64b<cp_async::PrefetchMode::kPrefetch, fill_mode>(
            reinterpret_cast<DType*>(drow + j * 64 + L * 8), gptr, kv_idx < kv_len);
 ...
 * Fragment/SF register mappings validated bit-exact by the fork's k3_unit.cu on sm120a.
 */
template <int TIDB>
 ...
 *  - Q: linear packed bytes in q_smem (see load_q_packed_global_smem), row stride HEAD_DIM_QK/2.
 *  - K: dense-packed fp4 (kDenseKFp4), row-major HEAD_DIM_QK/2 bytes/row (see produce_kv).
 *  - K SF: flat sf[kv_row * (HEAD_DIM_QK/16) + hd_group], warp-local base (as compute_qk).
 */
template <typename KTraits>
...
#pragma unroll
    ...
    // ---- odd-NUM_MMA_KV tail: one n16 fragment via sub-MMAs 0/1 only. Clamp SFB read into
    // the n16 tile (bn & 15) so dead lanes never overrun k_sf_smem into the in-flight v_sf_smem.
...
#pragma unroll
      ...
      if constexpr (KTraits::USE_NVF4_QK) {
        ...
        constexpr uint32_t Q_SF_COLS = KTraits::HEAD_DIM_QK / NVFP4_SF_VEC_SIZE;
...
#pragma unroll 1
      for (uint32_t iter = 0; iter < num_iterations; ++iter) {
        ...
        if constexpr (KTraits::USE_NVF4_QK) {
          // A4Q: native nvf4 block-scaled Q.K^T (dense-packed fp4 K + Q-SF, see compute_qk_nvf4).
...
#pragma unroll
      ...
      if constexpr (KTraits::USE_NVF4_QK) {
        ...
        constexpr uint32_t Q_SF_COLS = KTraits::HEAD_DIM_QK / NVFP4_SF_VEC_SIZE;
        const uint32_t q_sf_stride_n = num_qo_heads * Q_SF_COLS;
        const uint8_t* q_sf_ptr_base = params.maybe_q_sf + q_indptr[request_idx] * q_sf_stride_n +
